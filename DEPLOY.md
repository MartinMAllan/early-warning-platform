...
## Step 2: Paste the startup script into the box

The script lives in this repo as its own file: **[`startup-script.sh`](startup-script.sh)**.

1. Open `startup-script.sh` and edit the `REPO_URL` line near the top (nothing else needs to change).
2. Copy the whole file's contents.
3. Paste it into the Droplet creation screen's "Enter your scripts or data" box (Additional Options → Startup scripts).

It installs Python and git, clones this repo, installs `backend/requirements.txt`, and runs FastAPI as a systemd service (`earlywarning`) on port 80 with auto-restart, then opens the firewall on ports 22 and 80.
